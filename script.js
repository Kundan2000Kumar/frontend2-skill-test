...
function favourite(element){
	...
	for(let i = 0; i< currentMovieStack.length; i++){
		if(currentMovieStack[i].id == id){
			let favouriteMoviesAnuj = JSON.parse(localStorage.getItem("favouriteMoviesAnuj"));
			
			if(favouriteMoviesAnuj == null){
				favouriteMoviesAnuj = [];
			}

			favouriteMoviesAnuj.unshift(currentMovieStack[i]);
			localStorage.setItem("favouriteMoviesAnuj", JSON.stringify(favouriteMoviesAnuj));
...
goToFavouriteButton.addEventListener('click', ()=>{
	let favouriteMoviesAnuj = JSON.parse(localStorage.getItem("favouriteMoviesAnuj"));
	if(favouriteMoviesAnuj == null || favouriteMoviesAnuj.length < 1){
		showAlert("you have not added any movie to favourite");
		return;
	}

	currentMovieStack = favouriteMoviesAnuj;
	renderList("remove");
})
...
function remove(element){
	...
	for(let i = 0; i<favouriteMoviesAnuj.length; i++){
		if(favouriteMoviesAnuj[i].id == id){
			continue;
		}
		newFavouriteMovies.push(favouriteMoviesAnuj[i]);
	}
	
	localStorage.setItem("favouriteMoviesAnuj", JSON.stringify(newFavouriteMovies));
